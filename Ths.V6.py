...
# ==========================================
st.set_page_config(page_title="Thesis Master V6.1 (Humanis)", page_icon="🎓", layout="wide")

# --- KOLAM API KEY ---
# Biarkan kosong/dummy untuk GitHub. Key asli ada di Streamlit Secrets.
API_KEYS_POOL = [
    "KOSONGKAN_UNTUK_GITHUB", 
    "KEY_ASLI_ADA_DI_SECRETS_STREAMLIT"
]
...
def perform_full_reset():
    ...
    keys = ['naskah_bab1', 'naskah_bab2', 'naskah_bab3', 'naskah_bab4', 'naskah_bab5',
            ...
            'judul_final', 'data_penelitian', 'search_results', 'opsi_judul_generated']
    for k in keys:
...
def generate_search_formulas(title, bidang):
    ...
    TUGAS: Buatkan 3 Rumus Pencarian (Search String) untuk database jurnal internasional.
    
    Output WAJIB berupa List Python ['Rumus 1', 'Rumus 2', 'Rumus 3'].
    
    Kriteria:
    1. Rumus Basic: Keyword utama.
    2. Rumus Menengah: Gunakan sinonim.
    3. Rumus ADVANCED BOOLEAN: Gunakan ("Var1" OR "Sinonim") AND ("Var2").
    """
...
keys = ['naskah_bab1', 'naskah_bab2', 'naskah_bab3', 'naskah_bab4', 'naskah_bab5',
        'outline_bab1', 'outline_bab2', 'outline_bab3', 'outline_bab4', 'outline_bab5',
        'judul_final', 'pdf_context', 'data_penelitian', 'search_results', 'opsi_judul_generated', 'bidang_ilmu']
for k in keys:
    if k not in st.session_state:
        if k == 'pdf_context': st.session_state[k] = load_from_disk(PDF_CACHE_FILE)
        elif 'outline' in k or 'search' in k or 'opsi' in k: st.session_state[k] = []
...
st.markdown(f'<div class="main-header">🎓 Thesis Master V6.1</div>', unsafe_allow_html=True)
st.markdown(f'<div class="sub-header">Mode: {st.session_state["bidang_ilmu"]} | Humanist V6.1</div>', unsafe_allow_html=True)
st.divider()
        ...
        if st.session_state[key_outline]:
            ...
            with c_btn: 
                if st.button(f"🚀 Tulis", key=f"g_{key_naskah}"):
                    with st.spinner("Menulis (Humanized & Validated)..."):
                        ctx = retrieve_relevant_context(sub, st.session_state['pdf_context'])
                        
                        # --- INTELLIGENT PROMPT V6.1 ---
                        p = f"""
                        ...
                        1. SUMBER DATA NYATA: Gunakan HANYA data dan fakta yang ada di 'DATA PENDUKUNG (CONTEXT)' di atas. Jangan mengarang data.
                        2. FILTER TAHUN CERDAS: Jika ada kutipan tahun lama (misal 2013) di dalam teks Context, JANGAN jadikan itu data utama. Cari kalimat yang merupakan TEMUAN TERBARU dari jurnal tersebut (biasanya di bagian Hasil/Diskusi).
                        3. GAYA BAHASA HUMANIS (PENTING): Tulis dengan gaya natural manusia, gunakan variasi struktur kalimat, hindari repetisi kaku dan kata-kata klise AI/Robotik. Buat alur paragraf yang mengalir (koheren).
                        4. SITASI: Gunakan format APA (Nama, Tahun). DILARANG KERAS menggunakan angka [1], [2].
                        5. NO PROLOG: Langsung ke materi.
                        6. Panjang: 600-900 kata.
                        """
                        try:
